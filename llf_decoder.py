...
class LLFDecoder(nn.Module):
    def __init__(self, num_classes):
        ...
        # Projections for skip connections (LLFP1,LLFP2,HLFP1,HLFP2)
        ...
        self.proj3 = nn.Conv2d(320, embed_dim, kernel_size=3, padding=1, stride=1)  # Stage 3 projection [batch,320,32,32] -> [batch,128,32,32]
        ...
        self.proj4_1 = nn.Conv2d(512, embed_dim*2, kernel_size=3, padding=1, stride=1)  # Stage 4 projection 1 [batch,512,16,16] -> [batch,256,16,16]
        self.bn_proj4_1 = nn.BatchNorm2d(embed_dim*2)
        self.leakyReLU_proj4_1 = nn.LeakyReLU()
        
        self.proj4_2 = nn.Conv2d(embed_dim*2, embed_dim, kernel_size=3, padding=1, stride=1)  # Stage 4 projection 2 [batch,256,16,16] -> [batch,128,16,16]
        ...
        self.projSkips_1 = nn.Conv2d(512, embed_dim*2, kernel_size=1) # [batch, 512,128,128] -> [batch, 256,128,128] (512->448)
    ...
    def forward(self, encoder_outputs):
        # encoder_outputs come from segformer model 
        ...
        # Interpolations to match feature map size of x1 

        x2_up = F.interpolate(att_x2, size=(128, 128), mode='bilinear', align_corners=False) # [batch,128,64,64] -> [batch,128,128,128]

        x3_up = F.interpolate(x3, size=(64, 64), mode='bilinear', align_corners=False) # [b,128,32,32] -> [b, 128,64,64]
        x3_up = F.interpolate(x3_up, size=(128, 128), mode='bilinear', align_corners=False) # [batch,128,64,64] -> [batch,128,128,128]

        x4_up = F.interpolate(x4, size=(32, 32), mode='bilinear', align_corners=False) # [b,128,16,16] -> [b, 128,32,32]
        x4_up = F.interpolate(x4_up, size=(64, 64), mode='bilinear', align_corners=False) # [b,128,32,32] -> [b, 128,64,64]
        x4_up = F.interpolate(x4_up, size=(128, 128), mode='bilinear', align_corners=False) # [batch,128,64,64] -> [batch,128,128,128]
        ...
        # first and second pointwise convolutions from decoder
        concatenated_skips = self.projSkips_1(concatenated_skips) # [b, 512,128,128] -> [b, 256,128,128] 
        ...
        concatenated_skips = self.projSkips_2(concatenated_skips) # [b, 256,128,128] -> [b, 128,128,128]
